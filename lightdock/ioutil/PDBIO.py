...
class PDBIO(IO):
    ...
    def __init__(self):
        self.log = LoggingManager.get_logger("pdb")
    ...
    def read_atom_line(line, line_type="", atoms_to_ignore=None, residues_to_ignore=None):
        ...
        atom_name = IO.cstrip(line[12:16])
        atom_alternative = IO.cstrip(line[16])
        residue_name = IO.cstrip(line[17:21])
        chain_id = IO.cstrip(line[21])
    ...
    def write_atom_line(atom, atom_coordinates, output):
        ...
        line = "%6s%5d %-4s%-1s%3s%2s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f%12s%s" % (
            atom_type,
            atom.number,
            PDBIO._format_atom_name(atom.name),
            atom.alternative,
            atom.residue_name,
            atom.chain_id,
            atom.residue_number,
    ...
    def write_to_file(
        ...
        for atom in molecule.atoms:
            if atom_coordinates is not None:
                PDBIO.write_atom_line(atom, atom_coordinates, output_file)
            else:
                PDBIO.write_atom_line(atom, molecule.atom_coordinates[structure_id], output_file)
        output_file.close()
        ...
        with open(file_name, "w") as output:
            for index, point in enumerate(points):
                line = "ATOM  %5d %-4s %3s%2s%4d%1s   %8.3f%8.3f%8.3f%6.2f%6.2f%12s%s" % (
                    index + 1,
                    PDBIO._format_atom_name(atom_name),
                    res_name,
                    chain_id,
                    index + 1,
                    "",
