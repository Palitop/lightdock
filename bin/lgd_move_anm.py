...
"""Generates a set of n_confs conformations of an input file PDB/MMCIF structure
using the first non-trivial ANM n_modes in a given rmsd interval"""

import numpy as np
import argparse
from prody import parsePDB, ANM, extendModel, confProDy, sampleModes, writePDB, writeMMCIF
...
if __name__ == "__main__":
    ...
    molecule = parsePDB(args.file_name)
    ...
    io = IOFactory(args.file_name).get_instance()
    atoms, residues, chains = io.parse_complex_from_file(args.file_name)
    ...
    for i in range(args.n_confs):
        ...
        output_file = f"anm_{i+1}_{args.file_name}"
    ...
    if args.ensemble:
        ...
        ensemble_file_name = "ensemble." + args.output_format
        if args.output_format in ["cif", "mmcif"]:
            writeMMCIF(ensemble_file_name, backbone)
        else:
